    ...
    def fit(
        ...
        **kwargs,
        ...
        rstring = """
            ...
            out <- lapply(ranef(model, condVar = TRUE),uniquify)
            out <- lapply(out, getIndex)
            out
            # print(out)
            }
        ...
        rstring = """
            function(model){
            re_sim <- ranef(model, condVar=TRUE)                        
            postvar <- sqrt(attr(re_sim$stat_id, "postVar"))
            
            # postvar is shape (1,1,N) so convert to (N,1)
            postvar_vector <- as.vector(postvar)
            row_names <- rownames(re_sim$stat_id)
            
            # Create an R dataframe for the output            
            output_df <- data.frame(stat_id = rep(NA, length(postvar_vector)), postvar = rep(NA, length(postvar_vector)))
            output_df[['stat_id']] <- row_names
            output_df[['postvar']] <- postvar_vector
            output_df
            }
        """
        ranef_func2 = robjects.r(rstring)
        ranefs2 = ranef_func2(self.model_obj)

        self.postvar = list(zip(*ranefs2))
